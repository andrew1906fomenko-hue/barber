...
const mobileSwipeSections: Section[] = ["Главная", "Клиенты", "График работы", "Статистика", "Настройки"];
...
export default function DashboardPage() { const router = useRouter(); const today = useMemo(() => new Date(), []); const dashboardSwipeStageRef = useRef<HTMLDivElement | null>(null); const dashboardSwipeStart = useRef<{
    ...
    return;
} const [servicesResponse, appointmentsResponse, blockedTimesResponse] = await Promise.all([fetch("/api/services"), fetch("/api/appointments"), fetch("/api/blocked_times"),]); const servicesData = servicesResponse.ok ? ((await servicesResponse.json()) as {
    success: boolean;
    services?: Service[];
    ...
    const [appointmentsResponse, blockedTimesResponse] = await Promise.all([fetch("/api/appointments"), fetch("/api/blocked_times"),]);
...
function MobileWhatsAppBottomNav({ section, setSection, }: {
    ...
    label: string;
    icon: IconName;
}> = [{ section: "Главная", label: "Главная", icon: "home" }, { section: "Клиенты", label: "Клиенты", icon: "clients" }, { section: "График работы", label: "График", icon: "calendar" }, { section: "Статистика", label: "Статистика", icon: "chart" }, { section: "Настройки", label: "Настройки", icon: "settings" },]; return (<nav className="whatsapp-bottom-nav fixed bottom-0 left-0 right-0 z-30 md:hidden"> <div className="grid grid-cols-5 px-1.5 pb-[calc(env(safe-area-inset-bottom)+4px)] pt-1.5"> {items.map((item) => { const active = section === item.section || (item.section === "Статистика" && (section === "Аналитика" || section === "Финансы")); return (<button key={item.section} type="button" onClick={() => setSection(item.section)} className={`mobile-nav-item whatsapp-bottom-nav-item flex min-h-14 flex-col items-center justify-center gap-0.5 rounded-xl text-tabLabel ${active ? "mobile-nav-item-active whatsapp-bottom-nav-active" : ""}`}> <span className="relative flex h-7 w-10 items-center justify-center rounded-full"> <NavIcon name={item.icon} active={active}/> </span> <span>{item.label}</span></button>); })} </div> </nav>); }
...
function MobileWhatsAppHeader({ appointmentsCount, clientsCount, onAddClient, onAddService, onMarkNotificationsRead, notificationsOpen, selectedDate, selectedWeekDays, section, servicesCount, setNotificationsOpen, setSelectedDate, setSection, unreadNotificationsCount, }: {
    ...
    return (<header className="neutral-app-header whatsapp-mobile-header whatsapp-settings-mobile-header sticky top-0 z-30 md:hidden" style={{ background: "var(--surface)", backgroundColor: "var(--surface)", backgroundImage: "none", color: "var(--textPrimary)" }}> <div className="px-4 pb-3 pt-[calc(env(safe-area-inset-top)+12px)]"> <div className="flex items-center justify-between gap-3"> <h1 className="app-main-header-title whatsapp-header-title truncate text-navigationTitle">Настройки</h1> </div> </div> </header>);
...
function ChevronIcon({ open }: {
    open: boolean;
}) { return <CaretDown className={`h-5 w-5 transition-transform ${open ? "rotate-180" : ""}`} weight="bold" aria-hidden="true"/>; }
...
function HomeSection(props: {
    ...
    unreadNotificationsCount: number;
}) { const [deleteTarget, setDeleteTarget] = useState<Appointment | null>(null); const [selectedAppointmentId, setSelectedAppointmentId] = useState<string | null>(null); const [rescheduleModalOpen, setRescheduleModalOpen] = useState(false); const [rescheduleDraft, setRescheduleDraft] = useState({ date: "", time: "" }); const [noteModalOpen, setNoteModalOpen] = useState(false); const [noteDraft, setNoteDraft] = useState(""); const [expandedCompactAppointmentId, setExpandedCompactAppointmentId] = useState<string | null>(null); const [touchStart, setTouchStart] = useState<{
    x: number;
    y: number;
} | null>(null); const [calendarWeekRoll, setCalendarWeekRoll] = useState<{
    direction: -1 | 1;
    nonce: number;
} | null>(null); const [calendarMonthRoll, setCalendarMonthRoll] = useState<{
    direction: -1 | 1;
    nonce: number;
} | null>(null); const [statusNow, setStatusNow] = useState(() => Date.now()); const calendarWeekTouchStartRef = useRef<number | null>(null); const [appointmentDrag, setAppointmentDrag] = useState<{
    active: boolean;
    id: string;
    settling: boolean;
    scale: number;
    ...
    window.clearTimeout(appointmentDragResetRef.current); }; }, []); const startAppointmentDrag = (event: PointerEvent<HTMLElement>, id: string) => { if (event.button !== 0 && event.pointerType === "mouse")
...
function AnalyticsSection({ appointments, activeServices, blockedTimes, compact = false, services, }: {
    appointments: Appointment[];
    activeServices: number;
    blockedTimes: BlockedTime[];
    compact?: boolean;
    ...
    const loadPercent = periodAppointments.length + periodBlockedTimes.length ? Math.round((periodAppointments.length / (periodAppointments.length + periodBlockedTimes.length)) * 100) : 0;
    const previousLoadPercent = previousPeriodAppointments.length + previousPeriodBlockedTimes.length ? Math.round((previousPeriodAppointments.length / (previousPeriodAppointments.length + previousPeriodBlockedTimes.length)) * 100) : 0;
    ...
    const serviceStats = services.map((service) => ({ id: service.id, title: service.title, count: detailAppointments.filter((item) => getAppointmentServiceIds(item).includes(service.id)).length, })).filter((item) => item.count > 0).sort((a, b) => b.count - a.count).slice(0, 4);
    ...
    const sourceStats = Object.entries(detailAppointments.reduce<Record<string, number>>((counts, appointment) => { const source = getAppointmentSource(appointment); if (!source)
        return counts; const label = source.includes("online") || source.includes("онлайн") ? "Онлайн" : source.includes("recommend") || source.includes("рекомен") ? "По рекомендации" : source; counts[label] = (counts[label] || 0) + 1; return counts; }, {})).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([label, count], index) => ({ label, count, percent: detailAppointments.length ? Math.round((count / detailAppointments.length) * 100) : 0, tone: ["blue", "green", "orange"][index] || "blue", }));
    ...
    const selectPeriodMode = (mode: "day" | "week" | "month" | "year") => { setPeriodMode(mode); const nextRange = getPeriodRange(todayKey, mode); setSelectedChartKey(null); setSelectedPeakDayKey(formatDateKey(nextRange.start)); setSelectedPeakHour(null); analyticsUserScrollRef.current = false; setAnalyticsScrollNonce((current) => current + 1); };
    const selectChartItem = (item: {
        key: string;
        dateKey?: string;
    }) => { setSelectedChartKey(item.key); if (item.dateKey)
        setSelectedPeakDayKey(item.dateKey); setSelectedPeakHour(null); };
    ...
    return (<div className={compact ? "analytics-reference-screen analytics-reference-compact" : "analytics-reference-screen"}> <div className="analytics-reference-periods" role="tablist" aria-label="Период" onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}> {periodOptions.map((period) => (<button key={period.mode} type="button" className={periodMode === period.mode ? "is-active" : ""} onClick={() => selectPeriodMode(period.mode)} role="tab" aria-selected={periodMode === period.mode}> {period.label}</button>))} </div> <section className="analytics-reference-kpi-grid"> <AnalyticsMetric icon={<CalendarBlank weight="regular"/>} tone="violet" label="Записи" value={periodAppointments.length.toString()} delta={formatNumberDelta(periodAppointments.length, previousPeriodAppointments.length)}/> <AnalyticsMetric icon={<User weight="regular"/>} tone="blue" label="Новые клиенты" value={newClients.toString()} delta={formatNumberDelta(newClients, previousNewClients)}/> <AnalyticsMetric icon={<ChartLineUp weight="regular"/>} tone="green" label="Повторные" value={returningClients.toString()} delta={formatNumberDelta(returningClients, previousReturningClients)}/> <AnalyticsMetric icon={<Gauge weight="regular"/>} tone="orange" label="Загрузка" value={`${loadPercent}%`} delta={formatPercentDelta(loadPercent, previousLoadPercent)}/> </section> <article className="analytics-reference-card analytics-reference-chart-card finance-reference-chart-card"> <div className="analytics-reference-card-header"> <h2>Динамика записей</h2> </div> <div className="analytics-reference-bar-chart finance-chart-frame" data-dashboard-swipe-ignore="true"> <div ref={analyticsChartRef} className={`analytics-reference-chart-bars insights-chart finance-chart finance-chart-peek-window finance-chart-peek-${periodMode}`} onScroll={handleAnalyticsInfiniteScroll} onPointerDown={markAnalyticsChartScroll} onTouchStart={markAnalyticsChartScroll} onWheel={markAnalyticsChartScroll}> {chartData.map((item) => (<button key={item.key} type="button" className="analytics-reference-chart-column insights-chart-column finance-chart-column" data-date-key={item.key} data-empty={item.value === 0} data-active={item.isActive ? "true" : undefined} data-selected={selectedChartKey === item.key ? "true" : undefined} onClick={() => selectChartItem(item)} aria-label={`${item.label}: ${item.value} записей`}> <div className="insights-chart-bar-wrap" style={{ "--finance-bar-height": `${Math.max(item.value ? 10 : 3, (item.value / chartScaleMax) * 100)}%` } as CSSProperties}> <span>{item.value ? item.value.toString() : ""}</span> <div className="insights-chart-bar" style={{ height: "var(--finance-bar-height)" }}/> </div> <small className="finance-chart-date"><span>{item.label}</span> {item.dateLabel && <em>{item.dateLabel}</em>}</small></button>))} </div> </div> </article> {showAnalyticsDetails && (<> <section className="analytics-reference-kpi-grid analytics-reference-kpi-grid-secondary"> <AnalyticsMetric icon={<Wallet weight="regular"/>} tone="violet" label="Средний чек" value={`${detailAverageCheck.toLocaleString("ru-RU")} ₽`} delta={formatMoneyDelta(detailAverageCheck, previousAverageCheck)}/> <AnalyticsMetric icon={<ChartLineUp weight="regular"/>} tone="blue" label="Конверсия онлайн-записи" value={`${detailOnlineBookingPercent}%`} delta={formatPercentDelta(detailOnlineBookingPercent, previousOnlineBookingPercent)}/> <AnalyticsMetric icon={<XCircle weight="regular"/>} tone="rose" label="Отмены" value={detailCancellations.toString()} delta={formatNumberDelta(detailCancellations, previousCancellations)}/> <AnalyticsMetric icon={<Clock weight="regular"/>} tone="green" label="Среднее время услуги" value={`${detailAverageDuration} мин`} delta={formatNumberDelta(detailAverageDuration, previousAverageDuration, " мин")}/> </section> <article className="analytics-reference-card analytics-reference-services"> <div className="analytics-reference-card-header"> <h2>Популярные услуги</h2> <button type="button" className="analytics-reference-link">Смотреть все</button> </div> <div className="analytics-reference-service-list"> {serviceRows.length === 0 ? (<p className="analytics-reference-empty">Нет записей с услугами за выбранный период.</p>) : serviceRows.map((item) => (<div key={item.id} className="analytics-reference-service-row"> <span className={`analytics-reference-service-icon tone-${item.tone}`} aria-hidden="true">{item.title.slice(0, 1)}</span> <p>{item.title}</p> <div><span style={{ width: `${Math.max(14, (item.count / topServiceCount) * 100)}%` }}/></div> <strong>{item.count} записей</strong> <em>{item.percent}%</em> </div>))} </div> </article> <section className="analytics-reference-small-grid"> <article className="analytics-reference-card analytics-reference-clients"> <h2><Users className="h-4 w-4" weight="regular" aria-hidden="true"/> Клиенты</h2> <div className="analytics-reference-client-ring"> <div><span>Новые</span><strong>{detailNewPercent}%</strong><em>{detailNewClients} клиентов</em></div> <span className="analytics-reference-donut" style={{ "--returning": `${detailReturningPercent}%` } as CSSProperties} aria-hidden="true"/> <div><span>Постоянные</span><strong>{detailReturningPercent}%</strong><em>{detailReturningClients} клиента</em></div> </div> </article> <article className="analytics-reference-card analytics-reference-return"> <h2><ChartLineUp className="h-4 w-4" weight="regular" aria-hidden="true"/> Возвращаемость</h2> <strong>{detailReturningPercent}%</strong> <p>клиентов выбранного периода уже были в базе ранее</p> <span>{formatPercentDelta(detailReturningPercent, previousReturningPercent)}</span> </article> <article className="analytics-reference-card analytics-reference-sources"> <h2><Users className="h-4 w-4" weight="regular" aria-hidden="true"/> Топ источники записи</h2> {sourceStats.length === 0 ? (<p className="analytics-reference-empty">В записях не указан источник.</p>) : sourceStats.map(({ label, percent, tone }) => (<div key={label} className="analytics-reference-source-row"> <span className={`tone-${tone}`} aria-hidden="true"/> <p>{label}</p> <div><span style={{ width: `${percent}%` }}/></div> <strong>{percent}%</strong> </div>))} </article> </section> <article className="analytics-reference-card analytics-reference-peaks"> <h2><Clock className="h-4 w-4" weight="regular" aria-hidden="true"/> Пиковые часы</h2> <div className="analytics-reference-days"> {peakDays.map((day) => (<button key={day.key} type="button" className={day.key === selectedPeakDayKey ? "is-active" : ""} onClick={() => { setSelectedPeakDayKey(day.key); setSelectedPeakHour(null); }}> {day.label}</button>))} </div> <div className="analytics-reference-peak-chart"> <span className="analytics-reference-peak-bubble">{peakHourLabel}<br />{activePeakCount ? `${activePeakCount} записей` : "записей нет"}</span> {peakBars.map((value, index) => (<button key={hourLabels[index]} type="button" className={index === activePeakHourIndex ? "is-peak" : ""} data-empty={value === 0} style={{ height: value ? `${Math.max(8, (value / maxPeakCount) * 86)}px` : "0px" }} onClick={() => setSelectedPeakHour(hourLabels[index])} aria-label={`${hourLabels[index]}:00, ${value} записей`}/>))} </div> <div className="analytics-reference-peak-axis"><span>8:00</span><span>10:00</span><span>12:00</span><span>14:00</span><span>16:00</span><span>18:00</span><span>20:00</span></div> </article> </>)} </div>);
}
function AnalyticsMetric({ delta, icon, label, tone, value }: {
    delta: string;
    icon: ReactNode;
    label: string;
    tone: string;
    ...
    return (<article className="analytics-reference-metric"> <span className={`analytics-reference-metric-icon tone-${tone}`}>{icon}</span> <p>{label}</p> <strong>{value}</strong> <em className={`tone-${tone}`}>{delta}</em> </article>);
}
...
function FinanceSection({ appointments, compact = false, services, totalRevenue }: {
    ...
    const revenueByService = services.map((service) => { const matchingAppointments = summaryAppointments.filter((item) => getAppointmentServiceIds(item).includes(service.id)); const revenue = matchingAppointments.reduce((sum, appointment) => sum + getAppointmentPrice(appointment, [service]), 0); const profit = revenue - matchingAppointments.reduce((sum) => sum + (service.materialCost || 0), 0); return { id: service.id, title: service.title, count: matchingAppointments.length, revenue, profit }; }).filter((item) => item.revenue > 0).sort((a, b) => b.revenue - a.revenue).slice(0, 5);
...
function StatisticsSection({ appointments, activeServices, blockedTimes, services, totalRevenue, }: {
    appointments: Appointment[];
    activeServices: number;
    blockedTimes: BlockedTime[];
    services: Service[];
