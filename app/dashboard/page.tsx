...
import { ArrowBendUpLeft, ArrowLeft, Bell, CalendarBlank, CalendarCheck, CaretDown, CaretLeft, CaretRight, ChartLineUp, ChatText, Check, Clock, CurrencyRub, DotsThree, Eye, Folder, FunnelSimple, GearSix, Globe, Gauge, House, InstagramLogo, ListChecks, MapPin, Note, PencilSimple, Phone, Plus, MagnifyingGlass, SignOut, Palette, Receipt, SlidersHorizontal, TelegramLogo, TextAa, Trash, Tag, User, Users, Wallet, WhatsappLogo, X, XCircle, } from "@phosphor-icons/react";
...
function FreeSlotsPanel({ appointments, blockedTimes, open, selectedDate, services, setAppointmentForm, setOpen, setSelectedDate, setShowAppointmentForm, showToast, slotStepMin, weeklySchedule, workEnd, workStart, }: {
    ...
    const [cursorDate, setCursorDate] = useState(() => parseDateKey(selectedDate));
    const [shareSheetOpen, setShareSheetOpen] = useState(false);
    const dateWheelRef = useRef<HTMLDivElement | null>(null);
    const wheelScrollTimer = useRef<number | null>(null);
    useEffect(() => {
        if (open) {
            setCursorDate(parseDateKey(selectedDate));
            setShareSheetOpen(false);
        }
    }, [open, selectedDate]);
    const dayOptions = Array.from({ length: 21 }, (_, index) => addDays(parseDateKey(selectedDate), index - 10));
    const weekOptions = Array.from({ length: 13 }, (_, index) => addDays(parseDateKey(selectedDate), (index - 6) * 7));
    useEffect(() => {
        if (!open)
            return;
    ...
    const textToShare = allSlots.length ? `Свободные окошки:\n${displayGroups.filter((group) => group.slots.length).map((group) => `${formatLongDate(group.date)}: ${group.slots.map((slot) => slot.start).join(", ")}`).join("\n")}` : "Свободных окошек нет";
    const copySlots = async () => {
        try {
            await navigator.clipboard.writeText(textToShare);
            showToast("Свободные окошки скопированы");
        }
        catch {
            showToast(textToShare);
        }
    };
