"use client";
import { startTransition, useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState, type CSSProperties, type MouseEvent, type PointerEvent, type ReactNode, type RefObject, type TouchEvent } from "react";
import { useRouter } from "next/navigation";
import { createPortal } from "react-dom";
import { ArrowBendUpLeft, ArrowLeft, Bell, CalendarBlank, CalendarCheck, CaretDown, CaretLeft, CaretRight, ChartLineUp, ChatText, Check, Clock, CopySimple, CurrencyRub, DotsThree, Eye, Folder, FunnelSimple, GearSix, Globe, Gauge, House, InstagramLogo, ListChecks, MapPin, Note, PencilSimple, Phone, Plus, MagnifyingGlass, ShareNetwork, SignOut, Palette, Receipt, SlidersHorizontal, TelegramLogo, TextAa, Trash, Tag, User, Users, Wallet, WhatsappLogo, X, XCircle, } from "@phosphor-icons/react";
...
type FreeSlotRange = "day" | "week";
type FreeSlotsBackgroundId = "denim" | "graphite" | "blush" | "sage" | "pearl" | "berry" | "cocoa" | "lagoon" | "champagne" | "ink" | "sunset" | "mint";
type FreeSlotsBackgroundOption = {
    id: FreeSlotsBackgroundId;
    label: string;
    background: string;
    textColor: string;
    mutedColor: string;
...
const freeSlotsBackgroundOptions: FreeSlotsBackgroundOption[] = [
    { id: "ink", label: "Черника", background: "linear-gradient(145deg, #111827 0%, #27324b 45%, #6d7fa8 100%)", textColor: "#ffffff", mutedColor: "rgba(255, 255, 255, .72)" },
    { id: "denim", label: "Деним", background: "linear-gradient(145deg, #313a56 0%, #26314d 52%, #1d263f 100%)", textColor: "#ffffff", mutedColor: "rgba(255, 255, 255, .74)" },
    { id: "graphite", label: "Графит", background: "linear-gradient(145deg, #181a1f 0%, #2a2d34 48%, #111316 100%)", textColor: "#ffffff", mutedColor: "rgba(255, 255, 255, .7)" },
    { id: "blush", label: "Румянец", background: "linear-gradient(145deg, #ffe8ec 0%, #f7b7c5 46%, #9c5265 100%)", textColor: "#341721", mutedColor: "rgba(52, 23, 33, .72)" },
    { id: "sage", label: "Шалфей", background: "linear-gradient(145deg, #dcecdf 0%, #87b79a 48%, #285c49 100%)", textColor: "#10271f", mutedColor: "rgba(16, 39, 31, .72)" },
    { id: "pearl", label: "Перламутр", background: "linear-gradient(145deg, #fffaf2 0%, #dbe8ee 46%, #8ea8b4 100%)", textColor: "#17222a", mutedColor: "rgba(23, 34, 42, .68)" },
    { id: "berry", label: "Ягода", background: "linear-gradient(145deg, #412044 0%, #7e315f 50%, #d07b79 100%)", textColor: "#ffffff", mutedColor: "rgba(255, 255, 255, .75)" },
    { id: "cocoa", label: "Какао", background: "linear-gradient(145deg, #3a241c 0%, #7b4a35 48%, #d7a06d 100%)", textColor: "#fff7ef", mutedColor: "rgba(255, 247, 239, .74)" },
    { id: "lagoon", label: "Лагуна", background: "linear-gradient(145deg, #103f4b 0%, #16717f 48%, #79d7c7 100%)", textColor: "#f4fffb", mutedColor: "rgba(244, 255, 251, .74)" },
    { id: "champagne", label: "Шампань", background: "linear-gradient(145deg, #fff6db 0%, #ead08e 48%, #9a7240 100%)", textColor: "#2f2412", mutedColor: "rgba(47, 36, 18, .68)" },
    { id: "sunset", label: "Закат", background: "linear-gradient(145deg, #3c1d3d 0%, #d65f54 50%, #ffd08a 100%)", textColor: "#ffffff", mutedColor: "rgba(255, 255, 255, .78)" },
    { id: "mint", label: "Мята", background: "linear-gradient(145deg, #effcf6 0%, #a8e6cf 48%, #39a78e 100%)", textColor: "#12332c", mutedColor: "rgba(18, 51, 44, .68)" },
];
...
const getFreeSlotsForDate = (date: Date, appointments: Appointment[], blockedTimes: BlockedTime[], services: Service[], weeklySchedule: WeeklySchedule, fallbackStart: string, fallbackEnd: string, slotStepMin: number): FreeSlot[] => {
    const dateKey = formatDateKey(date);
    const schedule = getScheduleForDate(date, weeklySchedule, fallbackStart, fallbackEnd);
    const dayStart = timeToMinutes(schedule.start);
    const dayEnd = timeToMinutes(schedule.end);
    if (schedule.enabled === false || !Number.isFinite(dayStart) || !Number.isFinite(dayEnd) || dayStart >= dayEnd)
        return [];
    const busyIntervals = [
        ...getDayBreaks(schedule).filter(isBreakPeriodValid).map((item) => ({ start: timeToMinutes(item.start), end: timeToMinutes(item.end) })),
        ...blockedTimes.filter((item) => item.date === dateKey).map((item) => ({ start: timeToMinutes(item.start), end: timeToMinutes(item.end) })),
        ...appointments.filter((item) => item.date === dateKey && isLoadBearingAppointment(item)).map((item) => {
            const start = timeToMinutes(item.time);
            return { start, end: start + getAppointmentDuration(item, services) };
        }),
    ].map((item) => ({ start: Math.max(dayStart, item.start), end: Math.min(dayEnd, item.end) })).filter((item) => Number.isFinite(item.start) && Number.isFinite(item.end) && item.start < item.end).sort((left, right) => left.start - right.start);
    const freeSlots: FreeSlot[] = [];
    let cursor = dayStart;
    busyIntervals.forEach((interval) => {
        if (interval.start - cursor >= slotStepMin)
            freeSlots.push({ date: dateKey, start: minutesToTime(cursor), end: minutesToTime(interval.start) });
        cursor = Math.max(cursor, interval.end);
    });
    if (dayEnd - cursor >= slotStepMin)
        freeSlots.push({ date: dateKey, start: minutesToTime(cursor), end: minutesToTime(dayEnd) });
    return freeSlots;
};
...
function FreeSlotsPanel({ appointments, blockedTimes, open, selectedDate, services, setAppointmentForm, setOpen, setSelectedDate, setShowAppointmentForm, showToast, slotStepMin, weeklySchedule, workEnd, workStart, }: {
    ...
    const [range, setRange] = useState<FreeSlotRange>("week");
    const [cursorDate, setCursorDate] = useState(() => parseDateKey(formatDateKey(new Date())));
    const [shareSheetOpen, setShareSheetOpen] = useState(false);
    const [freeSlotsBackgroundId, setFreeSlotsBackgroundId] = useState<FreeSlotsBackgroundId>("ink");
    const dateWheelRef = useRef<HTMLDivElement | null>(null);
    const wheelScrollTimer = useRef<number | null>(null);
    useEffect(() => {
        const saved = window.localStorage.getItem("free-slots-background");
        if (freeSlotsBackgroundOptions.some((option) => option.id === saved))
            setFreeSlotsBackgroundId(saved as FreeSlotsBackgroundId);
    }, []);
    useEffect(() => {
        if (open) {
    ...
    const dayOptions = Array.from({ length: 21 }, (_, index) => addDays(todayDate, index));
    const weekOptions = Array.from({ length: 13 }, (_, index) => addDays(todayDate, index * 7));
    useEffect(() => {
        if (!open)
            return;
        const frame = window.requestAnimationFrame(() => {
            const wheel = dateWheelRef.current;
            const active = wheel?.querySelector<HTMLButtonElement>(".free-slots-date-active");
            active?.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
        });
        return () => window.cancelAnimationFrame(frame);
    }, [open, range, todayKey]);
    useEffect(() => () => {
        if (wheelScrollTimer.current !== null)
            window.clearTimeout(wheelScrollTimer.current);
    }, []);
    if (!open)
        return null;
    const activeDateKey = formatDateKey(cursorDate);
    const rangeDates = range === "day" ? [cursorDate] : getSelectedWeekDays(cursorDate);
    const groupedSlots = rangeDates.map((date) => ({ date, dateKey: formatDateKey(date), slots: getFreeSlotsForDate(date, appointments, blockedTimes, services, weeklySchedule, workStart, workEnd, slotStepMin) }));
    const displayGroups = groupedSlots.map((group) => ({ ...group, slots: group.slots.flatMap((slot) => {
        const start = timeToMinutes(slot.start);
        const end = timeToMinutes(slot.end);
        const slots: FreeSlot[] = [];
        for (let time = start; time + slotStepMin <= end; time += slotStepMin)
            slots.push({ date: slot.date, start: minutesToTime(time), end: minutesToTime(time + slotStepMin) });
        return slots;
    }) }));
    const allSlots = displayGroups.flatMap((group) => group.slots);
    const compactWeekSlots = range === "week" && allSlots.length > 24;
    const selectedDaySchedule = getScheduleForDate(cursorDate, weeklySchedule, workStart, workEnd);
    const emptySlotsText = range === "day" && getScheduleWorkMinutes(selectedDaySchedule) <= 0 ? "Выходной" : "Свободных окошек нет";
    const rangeTitle = range === "day" ? formatLongDate(cursorDate) : `${formatDateKey(groupedSlots[0]?.date || cursorDate).slice(8)}.${formatDateKey(groupedSlots[0]?.date || cursorDate).slice(5, 7)} - ${formatDateKey(groupedSlots[groupedSlots.length - 1]?.date || cursorDate).slice(8)}.${formatDateKey(groupedSlots[groupedSlots.length - 1]?.date || cursorDate).slice(5, 7)}`;
    const formatWheelDate = (date: Date) => {
    ...
    const updateCenteredDate = () => {
        ...
        const buttons = Array.from(wheel.querySelectorAll<HTMLButtonElement>("button[data-date]"));
        const closest = buttons.reduce<HTMLButtonElement | null>((best, button) => {
            if (!best)
                return button;
            const rect = button.getBoundingClientRect();
            const bestRect = best.getBoundingClientRect();
            return Math.abs(rect.left + rect.width / 2 - center) < Math.abs(bestRect.left + bestRect.width / 2 - center) ? button : best;
        }, null);
        const dateKey = closest?.dataset.date;
        if (dateKey && dateKey !== activeDateKey)
            setCursorDate(parseDateKey(dateKey));
    };
    const handleWheelScroll = () => {
        if (wheelScrollTimer.current !== null)
            window.clearTimeout(wheelScrollTimer.current);
        wheelScrollTimer.current = window.setTimeout(updateCenteredDate, 80);
    };
    const createAppointmentFromSlot = (slot: FreeSlot) => {
        setSelectedDate(slot.date);
        setAppointmentForm((current) => ({ ...current, time: slot.start }));
        setOpen(false);
        setShowAppointmentForm(true);
    };
    const selectBackground = (id: FreeSlotsBackgroundId) => {
        setFreeSlotsBackgroundId(id);
        window.localStorage.setItem("free-slots-background", id);
    };
    const activeBackground = freeSlotsBackgroundOptions.find((option) => option.id === freeSlotsBackgroundId) || freeSlotsBackgroundOptions[0];
    const freeSlotsCardStyle = {
        "--free-slots-bg": activeBackground.background,
        "--free-slots-text": activeBackground.textColor,
        "--free-slots-muted": activeBackground.mutedColor,
    } as CSSProperties;
    const visibleSlotGroups = displayGroups.filter((group) => group.slots.length);
    const getCompactWeekText = (slots: FreeSlot[]) => {
        const ranges: string[] = [];
        ...
        slots.slice(1).forEach((slot) => {
            ...
            ranges.push(start === previous.start ? start : `с ${start} по ${previous.start}`);
            start = slot.start;
            previous = slot;
        });
        ranges.push(start === previous.start ? start : `с ${start} по ${previous.start}`);
    ...
    const getPlainSlotRows = (slots: FreeSlot[]) => {
        const rows: string[] = [];
        for (let index = 0; index < slots.length; index += 4) {
            rows.push(`  ${slots.slice(index, index + 4).map((slot) => slot.start.padEnd(5, " ")).join("    ").trimEnd()}`);
        }
        return rows.join("\n");
    };
    const textToShare = allSlots.length ? [
        "Свободные окошки",
        range === "week" ? "на эту неделю" : "",
        "",
        ...visibleSlotGroups.map((group) => compactWeekSlots ? `${formatLongDate(group.date)}\n  ${getCompactWeekText(group.slots)}` : `${range === "day" ? rangeTitle : formatLongDate(group.date)}\n${getPlainSlotRows(group.slots)}`),
    ].filter((line, index, lines) => line || (index > 0 && lines[index - 1])).join("\n") : emptySlotsText;
    const htmlToShare = allSlots.length ? `<div style="background:${activeBackground.background};color:${activeBackground.textColor};font-family:Inter,Arial,sans-serif;padding:32px 24px;line-height:1.25;max-width:390px;"><div style="font-size:28px;font-weight:400;text-align:center;">Свободные окошки</div>${range === "week" ? `<div style="font-size:14px;font-weight:700;text-align:center;margin-top:2px;">на эту неделю</div>` : ""}<div style="margin-top:72px;display:grid;gap:${compactWeekSlots ? "16px" : "22px"};">${visibleSlotGroups.map((group) => compactWeekSlots ? `<div style="display:grid;grid-template-columns:110px 1fr;gap:14px;align-items:start;"><div style="color:${activeBackground.mutedColor};font-size:14px;font-weight:500;">${formatLongDate(group.date)}</div><div style="font-size:14px;font-weight:700;">${getCompactWeekText(group.slots)}</div></div>` : `<div><div style="color:${activeBackground.mutedColor};font-size:14px;font-weight:500;margin-bottom:14px;">${range === "day" ? rangeTitle : formatLongDate(group.date)}</div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:18px 22px;font-size:14px;font-weight:700;">${group.slots.map((slot) => `<span>${slot.start}</span>`).join("")}</div></div>`).join("")}</div></div>` : `<div style="font-family:Inter,Arial,sans-serif;">${emptySlotsText}</div>`;
    ...
    const copySlots = async () => {
        ...
        try {
            if (navigator.clipboard?.write && typeof ClipboardItem !== "undefined") {
                await navigator.clipboard.write([new ClipboardItem({
                        "text/html": new Blob([htmlToShare], { type: "text/html" }),
                        "text/plain": new Blob([textToShare], { type: "text/plain" }),
                    })]);
                copied = true;
            }
        ...
        if (!copied)
            copied = copyTextFallback(textToShare);
        showToast(copied ? "Текст скопирован" : "Не удалось скопировать текст");
    };
    const shareNative = async (label: string) => {
        setShareSheetOpen(false);
        if (navigator.share) {
            try {
                await navigator.share({ title: "Свободные окошки", text: textToShare });
                return;
            }
            catch {
                return;
        ...
        await copySlots();
        showToast(`${label}: текст скопирован`);
    };
    const shareUrl = (url: string) => {
        setShareSheetOpen(false);
        window.open(url, "_blank", "noopener,noreferrer");
    };
    return (<div className="free-slots-screen fixed inset-0 z-[150]" role="dialog" aria-modal="true" aria-labelledby="free-slots-title" data-dashboard-swipe-ignore="true"> <div className="free-slots-topbar"> <button type="button" onClick={() => setOpen(false)} className="free-slots-icon-button" aria-label="Закрыть" title="Закрыть"><X className="h-6 w-6" weight="regular" aria-hidden="true"/></button> <div className="free-slots-topbar-actions"> <button type="button" onClick={copySlots} className="free-slots-copy-button"><CopySimple className="h-4 w-4" weight="bold" aria-hidden="true"/><span>Скопировать текст</span></button> <button type="button" onClick={() => setShareSheetOpen(true)} className="free-slots-share-button"><ShareNetwork className="h-5 w-5" weight="bold" aria-hidden="true"/><span>Поделиться</span></button> </div> </div> <section className={`free-slots-card ${range === "week" ? "free-slots-card-week" : ""}`} style={freeSlotsCardStyle}> <header className="free-slots-hero"> <p id="free-slots-title">Свободные окошки</p> {range === "week" && <h2>на эту неделю</h2>} </header> <div className="free-slots-body"> {allSlots.length === 0 ? (<div className="free-slots-empty"> <p>{emptySlotsText}</p> </div>) : compactWeekSlots ? (<div className="free-slots-week-summary"> {displayGroups.filter((group) => group.slots.length).map((group) => (<article key={group.dateKey}> <h3>{formatLongDate(group.date)}</h3> <p>{getCompactWeekText(group.slots)}</p> </article>))} </div>) : (<div className={`free-slots-list ${range === "week" ? "free-slots-list-week-full" : ""}`}> {displayGroups.filter((group) => group.slots.length).map((group) => (<article key={group.dateKey} className="free-slots-day"> <h3>{range === "day" ? rangeTitle : formatLongDate(group.date)}</h3> <div>{group.slots.map((slot) => (<button key={`${slot.date}-${slot.start}-${slot.end}`} type="button" onClick={() => createAppointmentFromSlot(slot)}><span>{slot.start}</span></button>))}</div> </article>))} </div>)} </div> </section> <div className="free-slots-background-picker" role="listbox" aria-label="Выбор фона"> {freeSlotsBackgroundOptions.map((option) => (<button key={option.id} type="button" onClick={() => selectBackground(option.id)} className={option.id === activeBackground.id ? "free-slots-background-active" : ""} role="option" aria-selected={option.id === activeBackground.id} title={option.label}> <span style={{ background: option.background }} aria-hidden="true"/> <strong>{option.label}</strong> </button>))} </div> <div ref={dateWheelRef} className="free-slots-date-wheel" role="listbox" aria-label={range === "day" ? "Выбор дня" : "Выбор недели"} onScroll={handleWheelScroll}> {(range === "day" ? dayOptions : weekOptions).map((date) => { const dateKey = formatDateKey(date); const week = getSelectedWeekDays(date); const label = range === "day" ? formatWheelDate(date) : `${formatWheelDate(week[0])} - ${formatWheelDate(week[6])}`; const selected = range === "day" ? dateKey === activeDateKey : formatDateKey(week[0]) === formatDateKey(getSelectedWeekDays(cursorDate)[0]); return (<button key={`${range}-${dateKey}`} data-date={dateKey} type="button" onClick={() => setCursorDate(date)} className={selected ? "free-slots-date-active" : ""} role="option" aria-selected={selected}> <span>{label}</span> </button>); })} </div> <div className="free-slots-mode-tabs" role="tablist" aria-label="Формат периода"> {(["day", "week"] as FreeSlotRange[]).map((item) => (<button key={item} type="button" onClick={() => setRange(item)} className={range === item ? "free-slots-mode-active" : ""} role="tab" aria-selected={range === item}>{item === "day" ? "День" : "Неделя"}</button>))} </div> {shareSheetOpen && (<div className="free-slots-share-sheet" role="dialog" aria-modal="true" aria-label="Выбор платформы" onClick={() => setShareSheetOpen(false)}> <section onClick={(event) => event.stopPropagation()}> <div className="free-slots-share-handle" aria-hidden="true"/> <h3>Поделиться свободными окнами</h3> <button type="button" onClick={() => void shareNative("Instagram Stories")}><InstagramLogo className="h-5 w-5" weight="bold" aria-hidden="true"/><span>Instagram Stories</span></button> <button type="button" onClick={() => shareUrl(`https://wa.me/?text=${encodeURIComponent(textToShare)}`)}><WhatsappLogo className="h-5 w-5" weight="bold" aria-hidden="true"/><span>WhatsApp Stories</span></button> <button type="button" onClick={() => shareUrl(`https://t.me/share/url?text=${encodeURIComponent(textToShare)}`)}><TelegramLogo className="h-5 w-5" weight="bold" aria-hidden="true"/><span>Telegram Stories</span></button> <button type="button" onClick={() => void shareNative("Другие приложения")}><DotsThree className="h-5 w-5" weight="bold" aria-hidden="true"/><span>Другие приложения</span></button> </section> </div>)} </div>);
}
...
function DurationWheelPicker({ onChange, value }: {
    ...
    const selectedMinutes = Math.min(55, Math.max(0, Math.round((totalMinutes % 60) / 5) * 5));
    const hourOptions = useMemo(() => Array.from({ length: Math.max(13, selectedHours + 3) }, (_, index) => index), [selectedHours]);
    ...
    const timersRef = useRef<{ hours: ReturnType<typeof setTimeout> | null; minutes: ReturnType<typeof setTimeout> | null }>({ hours: null, minutes: null });
    const userScrollRef = useRef({ hours: false, minutes: false });
    useEffect(() => {
        hoursRef.current?.scrollTo({ top: Math.max(0, hourOptions.indexOf(selectedHours)) * itemHeight, behavior: "smooth" });
        minutesRef.current?.scrollTo({ top: Math.max(0, minuteOptions.indexOf(selectedMinutes)) * itemHeight, behavior: "smooth" });
    }, [hourOptions, minuteOptions, selectedHours, selectedMinutes]);
    useEffect(() => () => {
        if (timersRef.current.hours)
            clearTimeout(timersRef.current.hours);
        if (timersRef.current.minutes)
            clearTimeout(timersRef.current.minutes);
    }, []);
    const selectPart = (part: "hours" | "minutes", index: number) => {
        const nextHours = part === "hours" ? hourOptions[Math.min(Math.max(index, 0), hourOptions.length - 1)] : selectedHours;
        const nextMinutes = part === "minutes" ? minuteOptions[Math.min(Math.max(index, 0), minuteOptions.length - 1)] : selectedMinutes;
        onChange(nextHours * 60 + nextMinutes);
    };
    const handleScroll = (part: "hours" | "minutes") => {
        if (!userScrollRef.current[part])
            return;
        const list = part === "hours" ? hoursRef.current : minutesRef.current;
        if (!list)
            return;
        const options = part === "hours" ? hourOptions : minuteOptions;
        const nextIndex = Math.min(Math.max(Math.round(list.scrollTop / itemHeight), 0), options.length - 1);
        if (timersRef.current[part])
            clearTimeout(timersRef.current[part]);
        timersRef.current[part] = setTimeout(() => {
            selectPart(part, nextIndex);
            userScrollRef.current[part] = false;
        }, 70);
    };
    const renderColumn = (part: "hours" | "minutes", label: string, unit: string, options: number[], selected: number, ref: RefObject<HTMLDivElement | null>) => (<div className="service-duration-wheel-column"> <span className="service-duration-wheel-label">{label}</span> <div ref={ref} className="service-duration-wheel-list" style={{ height: itemHeight * visibleItems }} onScroll={() => handleScroll(part)} onPointerDown={() => { userScrollRef.current[part] = true; }} onTouchStart={() => { userScrollRef.current[part] = true; }} onWheel={() => { userScrollRef.current[part] = true; }} role="listbox" aria-label={label}> <div style={{ height: itemHeight }} aria-hidden="true"/> {options.map((option, index) => { const active = option === selected; return (<div key={option} role="option" aria-selected={active} className={`service-duration-wheel-option ${active ? "is-active" : ""}`} style={{ height: itemHeight }} onClick={() => selectPart(part, index)}> <strong>{option}</strong><span>{unit}</span> </div>); })} <div style={{ height: itemHeight }} aria-hidden="true"/> </div> <span className="service-duration-wheel-selection" aria-hidden="true"/> </div>);
    return (<div className="service-duration-wheel-picker" data-dashboard-swipe-ignore="true"> {renderColumn("hours", "Часы", "ч", hourOptions, selectedHours, hoursRef)} {renderColumn("minutes", "Минуты", "мин", minuteOptions, selectedMinutes, minutesRef)} </div>);
}
function ServicesSection(props: {
    ...
    services: Service[];
} => Boolean(item)); const popularSummary = serviceCategories.slice(0, 3).join(", ") || "Добавьте категории"; const selectedService = useMemo(() => services.find((service) => service.id === selectedServiceId) || null, [services, selectedServiceId]); const openServiceEditor = (service: Service) => { setSelectedServiceId(service.id); props.setServiceOverlayOpen(true); props.editService(service); }; const selectedServiceStats = useMemo(() => { if (!selectedService)
...
function ScheduleSection(props: {
    ...
    return (<div className={`schedule-settings-home max-w-5xl space-y-3 ${props.selectedWorkHoursDate ? "date-work-hours-overlay-only" : ""}`}> 
        ...
        {props.schedulePanel === "individual" && (<div className={`individual-schedule-screen fixed inset-0 z-50 overflow-y-auto bg-surface ${workHoursEditorOpen ? "individual-work-hours-screen weekday-booksy" : ""} ${props.selectedWorkHoursDate ? "date-work-hours-sheet-motion" : ""}`} data-dashboard-swipe-ignore="true">
            {workHoursEditorOpen ? workHoursEditorContent : (<>
